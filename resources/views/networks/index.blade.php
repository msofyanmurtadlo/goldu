...
@section('nav')
    <div class="flex-wrap d-flex justify-content-between align-items-center">
        <div>
            <h1>Networks</h1>
            <p>Displays Networks</p>
        </div>
        <div>
            <a class="btn btn-link btn-soft-light" onclick="createData('{{ route('networks.store') }}')">
                <i class="fa-solid fa-diagram-project"></i> Add Network
            </a>
        </div>
    </div>
@endsection
                    ...
                    <div class="card">
                        ...
                        <div class="card-body px-0">
                            <div class="table-responsive">
                                <table id="network-list-table" class="table table-striped" role="grid"
                                                ...
                                                <td>
                                                    <div class="flex align-items-center list-network-action">
                                                        <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip"
                                                            ...
                                                            onclick="editData('{{ route('networks.edit', $u->id) }}')">
                                                        ...
                                                        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip"
                                                            ...
                                                            onclick="deleteData('{{ route('networks.destroy', $u->id) }}')">
                                                            <span class="btn-inner">
                                                                <svg class="icon-20" width="20" viewBox="0 0 24 24"
                                                                    fill="none" xmlns="http://www.w3.org/2000/svg"
                                                                    stroke="currentColor">
    ...
    <script>
        ...
        function refreshTableContent(url = location.href) {
            $.ajax({
                ...
                url: url,
                success: function(response) {
                    var content = $(response).find('#network-list-table').parent();
                    $('#network-list-table').parent().replaceWith(content);
                },
                error: handleAjaxError
            });
        }
        ...
        $(document).ready(function() {
            ...
            $("#Input_Id").change(function() {
                ...
                var url = '{{ route('networks.index') }}?search=' + searchValue;
                refreshTableContent(url);
            });
        ...
        function createData(url) {
            ...
            $('#formModal .modal-title').text('Add network');
            $('#formModal form')[0].reset();
        ...
        function editData(url) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#formModal [name=subid]').val(response.subid);

                    // Menampilkan modal
                    $('#formModal').modal('show');
                    $('#formModal .modal-title').text('Edit network');
                    // Mengatur action form modal ke route update
                    var updateUrl = '{{ route('networks.update', ':id') }}'; // Tentukan route update Anda
                    updateUrl = updateUrl.replace(':id', response
                        .id); // Gantikan ":id" dengan id network yang akan diupdate
        ...
        function deleteData(url) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'You will delete this network.',
                icon: 'warning',
