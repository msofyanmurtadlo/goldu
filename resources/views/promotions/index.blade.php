...
@section('nav')
    <div class="flex-wrap d-flex justify-content-between align-items-center">
        <div>
            <h1>Promotions</h1>
            <p>Displays Promotions </p>
        </div>
        <div>
            <a class="btn btn-link btn-soft-light" onclick="createData('{{ route('promotions.store') }}')">
                <i class="fa-solid fa-rectangle-ad"></i> Add Promotions
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <div class="input-group">
                                ...
                                <input id="search" class="form-control" placeholder="Search Promotion">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body px-0">
                        <div class="table-responsive">
                            <table id="promotion-list-table" class="table table-striped" role="grid"
                                ...
                                <tbody>
                                    @foreach ($promotions as $u)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="flex align-items-center list-promotion-action">
                                                    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip"
                                                        ...
                                                        onclick="editData('{{ route('promotions.edit', $u->id) }}')">
                                                    ...
                                                    <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip"
                                                        ...
                                                        onclick="deleteData('{{ route('promotions.destroy', $u->id) }}')">
                                                        <span class="btn-inner">
                                                            <svg class="icon-20" width="20" viewBox="0 0 24 24"
                                                                fill="none" xmlns="http://www.w3.org/2000/svg"
                                                                stroke="currentColor">
    ...
    <script>
        ...
        function refreshTableContent(url = location.href) {
            $.ajax({
                ...
                url: url,
                success: function(response) {
                    var content = $(response).find('#promotion-list-table').parent();
                    $('#promotion-list-table').parent().replaceWith(content);
                },
                error: handleAjaxError
            });
        }
        ...
        $(document).ready(function() {
            ...
            $('#search').on('keyup', function() {
                ...
                var url = '{{ route('promotions.index') }}?search=' + searchValue;
                refreshTableContent(url);
            });
        });
        ...
        function createData(url) {
            ...
            $('#formModal .modal-title').text('Add promotion');
            $('#formModal form')[0].reset();
        ...
        function editData(url) {
            $.ajax({
                ...
                url: url,
                success: function(response) {
                    // Set data into form elements in the modal
                    $('#formModal [name=name]').val(response.name);
                    $('#formModal [name=link]').val(response.link);
                    // Show modal
                    $('#formModal').modal('show');
                    $('#formModal .modal-title').text('Edit Domain');
                    // Set form action in the modal to the update route
                    var updateUrl = '{{ route('promotions.update', ':id') }}'; // Specify your update route
                    updateUrl = updateUrl.replace(':id', response
                        .id); // Replace ":id" with the id of the Domain to be updated
        ...
        function deleteData(url) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'You will delete this promotion.',
                icon: 'warning',
