...
@section('nav')
    <div class="flex-wrap d-flex justify-content-between align-items-center">
        <div>
            <h1>Users</h1>
            <p>Displays all registered users</p>
        </div>
        <div>
            <a class="btn btn-link btn-soft-light" onclick="createData('{{ route('users.store') }}')">
                <i class="icon">
                            ...
                            <tbody>
                                @foreach ($users as $u)
                                    <tr>
                                        ...
                                        <td>
                                            <span
                                                class="badge bg-primary">{{ $u->is_active == 1 ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td>
                                            <div class="flex align-items-center list-user-action">
                                                <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip"
                                                    ...
                                                    onclick="editData('{{ route('users.edit', $u->id) }}')">
                                                ...
                                                <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip"
                                                    ...
                                                    onclick="deleteData('{{ route('users.destroy', $u->id) }}')">
                                                    <span class="btn-inner">
                                                        <svg class="icon-20" width="20" viewBox="0 0 24 24"
                                                            fill="none" xmlns="http://www.w3.org/2000/svg"
                                                            stroke="currentColor">
    ...
    <script>
        ...
        function refreshTableContent(url = location.href) {
            $.ajax({
                ...
                url: url,
                success: function(response) {
                    var content = $(response).find('#user-list-table').parent();
                    $('#user-list-table').parent().replaceWith(content);
                },
                error: handleAjaxError
            });
        }
        ...
        $(document).ready(function() {
            ...
            document.getElementById('custom_fee').addEventListener('change', function() {
                var feeField = document.getElementById('feeField');
                // Show the "Fee" input field if "Yes" is selected, otherwise hide it
                feeField.style.display = this.value === '1' ? 'block' : 'none';
            });
            $('#search').on('keyup', function() {
                var searchValue = $(this).val();
                var url = '{{ route('users.index') }}?search=' + searchValue;
                refreshTableContent(url);
            });
        });
        ...
        function createData(url) {
            ...
            $('#formModal .modal-title').text('Add User');
            $('#formModal form')[0].reset();
            $('#formModal form').attr('action', url);
            $('#formModal [name=_method]').val('post');
            $('#formModal [name=username]').prop('disabled', false);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.custom_fee == '1') {
                        ...
                        $('#feeField').hide(); // Hide the "Fee" input field
                    }
                    $('#formModal [name=fee]').val(response.fee);
                    $('#formModal [name=is_active]').val(response.is_active);
                    // Menampilkan modal
                    $('#formModal').modal('show');
                    $('#formModal .modal-title').text('Edit User');
                    // Mengatur action form modal ke route update
                    var updateUrl = '{{ route('users.update', ':id') }}'; // Tentukan route update Anda
                    updateUrl = updateUrl.replace(':id', response
                        .id); // Gantikan ":id" dengan id user yang akan diupdate
